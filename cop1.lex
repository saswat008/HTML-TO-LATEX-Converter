...
%x fontStart fontMiddle ahStart ahMiddle srcStart srcPath hStart hSize wStart wSize comStart comEnd symbolStart symbolMiddle
%option caseless
wSpaceChars	[ \t\n]+
strChars	[a-zA-Z0-9/.: ,;\\?'"!*\-|()]+
specialChars	[{}_\^@$\\%~#]
stringChars	{strChars}({wSpaceChars}{strChars})*
...
<symbolStart>[A-Za-z]+							{yylval.v = strdup(yytext);
									BEGIN symbolMiddle;
									return SYMBOLM;}

<symbolMiddle>";"							{yylval.v = strdup(";");
									BEGIN INITIAL;
									return SYMBOLE;}

{specialChars}								{yylval.v = strdup(yytext);
									return SPECIAL;}
